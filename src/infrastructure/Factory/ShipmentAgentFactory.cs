...
{
    internal class ShipmentAgentFactory(IChatClient chatClient, ShipmentPlugin plugin, IServiceProvider serviceProvider) : IShipmentAgentFactory
    {
        public AIAgent Create(string userId, string conversationId)
        {
            // TODO: Load conversation-specific chat history from database
            // var chatHistory = await _chatHistoryService.LoadHistoryAsync(userId, conversationId);
